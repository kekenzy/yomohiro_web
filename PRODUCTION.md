...
| `ALLOWED_HOSTS` | 許可するホスト（カンマ区切り） | `yomohiro.com,www.yomohiro.com,54.x.x.x` など。未設定時は `settings.py` の既定に本番ドメインを含む |
| `CSRF_TRUSTED_ORIGINS` | HTTPS オリジン（カンマ区切り、スキーム付き） | `https://yomohiro.com,https://www.yomohiro.com`。未設定時は `settings.py` の既定を使用 |
...
ALLOWED_HOSTS=yomohiro.com,www.yomohiro.com,（サーバIPがあれば追加）
